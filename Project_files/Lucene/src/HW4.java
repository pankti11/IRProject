...
public class HW4 {
	...
	HashMap<String, Integer> termFrq = new HashMap<String,Integer>(); //stores unique words and their frequency
	
	termFrq = populateWordFrequency(reader, j, count, termFrq);
	sortedTermFreq=sortWordFrequency(termFrq,false);
    //writeIndexToFile(sortedTermFreq); //writes index to file
	...
	File folder = new File("workbooks-lucene");
	folder.mkdir();
	...
	try (BufferedReader br1 = new BufferedReader(new FileReader(file))) {
		...
		while ((line = br1.readLine()) != null) {
			...
			try {
				...
				// 4. display results
				//displayResults(hits,searcher,q_no, "LuceneQuery"+q_no);
				...
				int termFreq = 0;
				int docCount=0;
				for (int i = 0; i < queryData.length; i++) {  //for each term in the input query
					Term termInstance = new Term("contents", queryData[i]); // s is the input query
	...
	try (FileOutputStream outputStream = new FileOutputStream("LuceneCACM_Relevant.xls")) {
    ...
    public static void writeExcelFile(ScoreDoc[] hits, IndexSearcher searcher, int qCount, String fileName){
    	...
    	HSSFSheet sheet = workbook.createSheet(""+qCount);
    	 int rowCount = 0;
    	 // doc id
    ...
    public static void displayResults(ScoreDoc[] hits, IndexSearcher searcher, int qCount, String fileName){
    	...
    	try {
    		...
    		// query_id Q0 doc_id rank CosineSim_score system_name
    		for (int i = 0; i < hits.length; ++i) {
    			int docId = hits[i].doc; // doc id
    			Document d;
    			d = searcher.doc(docId);
    			fw.write(String.format(formatDouble, qCount,"Q0",docId, (i+1),hits[i].score,"lucene-sys"));
    			
    			int  lastindex = d.get("path").lastIndexOf("\\");
    			String fName = d.get("path").substring(lastindex+1);
    			mapping.put(docId, fName);
    			fw1.write(String.format(formatDouble1, qCount,"Q0",fName, (i+1),hits[i].score,"lucene-sys"));
			...
			for(int docID : mapping.keySet()){ // get document name for the corresponding doc id
