...
def create_index():
	...
	count = 0
	TOTAL_DOCUMENT = 0
	for i in glob.glob("Stemmed-version/*.html"): #parsed files are stored in this folder.
		...
		hand = open("Stemmed-version/" + filename)
...
def create_model():
	...
	hand = open("cacm_stem.query.txt") #"query.txt" is the text file conating the queries.
	QueryList=[]
	queryid = 0
	...
	for line in hand:
		...
		saveToWorksheet(worksheet,bm25,queryid,term_docid_table,"bm-25-stemmed")
	workbook.close()
